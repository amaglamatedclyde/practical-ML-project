...
used for assessment of the generalization error of the final chosen model."*[^mynote2]: This is astounding to me, but you can try it yourself thanks to Reproducible Research.
</div>
...
We train on a set of **14220** observations in the **training set** and evaluate predictions on the **3473** observations in the validation set. The confusion matrix for the validation set is shown below. Note that there are only 2 misclasssifications.[^mynote2]

[^mynote2]: This is astounding to me, but you can try it yourself thanks to Reproducible Research.
...
```
####Performance
This investigator was fully prepared to cross-validate model tuning parameters and stack multiple models to improve prediction[^mynote3]. However, the **99.94% accuracy on the validation set** achieved using the xgboost algorithm with *default* settings led directly to an assessment on the test set. 

The test set performed even better, with 0 misclassifications. The **95% confidence interval** was shown to be **(0.9981, 1) on the hold-out test set of 1929 observations.** At this point it seemed foolish not to attempt the prediction portion of the course assessment. This resulted in 20 correct predictions out of 20 trials.

[^mynote3]: Seriously- you can't get beter than accuracy of 1 (see test set accuracy). Aside from the pleasure of an intellectual pursuit, why go further? 
...
Admittedly, the success of this effort is mostly attributable to the power of the XGBoost algorithm, elements of which are something of a black box to this investigator[^mynote3]. It can be assumed, however, that the predictor selection process (my part) was also crucial to the success of the algorithm.

[^mynote3]:Here is another interesting article on gradient boosting: http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3885826/
